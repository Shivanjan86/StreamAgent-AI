...
def fetch_live_web_sources(query: str, topic: str, sub_question_id: int) -> List[Dict[str, Any]]:
    """Fetch web research results using Wikipedia API, DuckDuckGo, or LLM Web Intelligence."""
    sources = []

    # 1. Attempt Wikipedia Live API Search (Ultra Fast & Unblocked on Datacenter IPs)
    try:
        encoded_query = urllib.parse.quote(f"{topic} {query}")
        headers = {"User-Agent": "StreamAgent-AI/1.0 (https://streamagent-ai.vercel.app)"}
        wiki_url = f"https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch={encoded_query}&utf8=&format=json"
        resp = requests.get(wiki_url, headers=headers, timeout=1.5)
        if resp.status_code == 200:
            ...
            for item in search_items[:3]:
                ...
                wiki_link = f"https://en.wikipedia.org/wiki/{urllib.parse.quote(title.replace(' ', '_'))}"
        ...
        logger.warning(f"[Searcher] Wikipedia API exception: {e}")

    # 2. Attempt DuckDuckGo search (Fallback)
    if not sources:
        try:
            encoded_query = urllib.parse.quote(query)
            headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
            resp = requests.get(f"https://html.duckduckgo.com/html/?q={encoded_query}", headers=headers, timeout=1.0)
